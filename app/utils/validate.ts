import { contains, mergeDeepLeft } from 'ramda'
 ...
 * 密码校验
 */
Validate.validators.type.types.password = function (value) {
  // /(?![0-9]+$)(?![a-z]+$)(?![A-Z]+$)(?!([^(0-9a-zA-Z)])+$).{8,16}/
  return /[a-zA-Z]/.test(value) && /\d/.test(value) && /^.{8,16}$/.test(value)
}
Validate.validators.type.messages.password = "密码格式有误";
/**
 ...
 * @param params
 */
export const validator = (params: ValidatorParams) => (data) => {
  const { rules = {}, extend } = params
  return validate(mergeDeepLeft(rules, getRules(extend)), data || {})
}
